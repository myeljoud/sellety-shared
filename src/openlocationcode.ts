...
export class OpenLocationCode {
  ...
  static readonly CODE_PRECISION_NORMAL = 10;
  static readonly CODE_PRECISION_EXTRA = 11;
  ...
  private static readonly ENCODING_BASE_ =
    OpenLocationCode.CODE_ALPHABET_.length;
  ...
  private static readonly PAIR_PRECISION_ = Math.pow(
    OpenLocationCode.ENCODING_BASE_,
    3
  );
  ...
  private static readonly GRID_CODE_LENGTH_ =
    OpenLocationCode.MAX_DIGIT_COUNT_ - OpenLocationCode.PAIR_CODE_LENGTH_;
  ...
  public static isValid(code: string): boolean {
    ...
    if (
      code.indexOf(OpenLocationCode.SEPARATOR_) !==
      code.lastIndexOf(OpenLocationCode.SEPARATOR_)
    ) {
      return false;
    }
    ...
    if (code.indexOf(OpenLocationCode.PADDING_CHARACTER_) > -1) {
      ...
      const padMatch = code.match(
        new RegExp("(" + OpenLocationCode.PADDING_CHARACTER_ + "+)", "g")
      );

      if (!padMatch) return false;
    ...
    for (let i = 0, len = strippedCode.length; i < len; i++) {
      ...
      if (
        character !== OpenLocationCode.SEPARATOR_ &&
        OpenLocationCode.CODE_ALPHABET_.indexOf(character) === -1
      ) {
        return false;
      }
    }
  ...
  public static isFull(code: string): boolean {
    ...
    const firstLatValue =
      OpenLocationCode.CODE_ALPHABET_.indexOf(code.charAt(0).toUpperCase()) *
      OpenLocationCode.ENCODING_BASE_;
    ...
    if (code.length > 1) {
      ...
      const firstLngValue =
        OpenLocationCode.CODE_ALPHABET_.indexOf(code.charAt(1).toUpperCase()) *
        OpenLocationCode.ENCODING_BASE_;
  ...
  public static encode(
    ...
    if (
      codeLength < 2 ||
      (codeLength < OpenLocationCode.PAIR_CODE_LENGTH_ && codeLength % 2 === 1)
    ) {
      throw new Error(
    ...
    const editedCodeLength = Math.min(
      OpenLocationCode.MAX_DIGIT_COUNT_,
      codeLength
    );
    ...
    if (editedLatitude === 90) {
      editedLatitude =
        editedLatitude -
        OpenLocationCode.computeLatitudePrecision(editedCodeLength);
    }
    let code = "";
    ...
    let latVal = Math.floor(
      Math.round(
        (editedLatitude + OpenLocationCode.LATITUDE_MAX_) *
          OpenLocationCode.FINAL_LAT_PRECISION_ *
          1e6
      ) / 1e6
    );
    let lngVal = Math.floor(
      Math.round(
        (editedLongitude + OpenLocationCode.LONGITUDE_MAX_) *
          OpenLocationCode.FINAL_LNG_PRECISION_ *
          1e6
      ) / 1e6
    );
    ...
    if (editedCodeLength > OpenLocationCode.PAIR_CODE_LENGTH_) {
      ...
      latVal = Math.floor(
        latVal /
          Math.pow(
            OpenLocationCode.GRID_ROWS_,
            OpenLocationCode.GRID_CODE_LENGTH_
          )
      );
      lngVal = Math.floor(
        lngVal /
    ...
    for (let i = 0; i < OpenLocationCode.PAIR_CODE_LENGTH_ / 2; i++) {
      code =
        OpenLocationCode.CODE_ALPHABET_.charAt(
          lngVal % OpenLocationCode.ENCODING_BASE_
        ) + code;
      code =
        OpenLocationCode.CODE_ALPHABET_.charAt(
          latVal % OpenLocationCode.ENCODING_BASE_
        ) + code;
    ...
    return (
      ...
      Array(OpenLocationCode.SEPARATOR_POSITION_ - editedCodeLength + 1).join(
        OpenLocationCode.PADDING_CHARACTER_
      ) +
      OpenLocationCode.SEPARATOR_
    );
  }
  ...
  public static decode(code: string): CodeArea {
    ...
    const editedCode = code
      ...
      .replace(
        new RegExp(
          OpenLocationCode.PADDING_CHARACTER_ + OpenLocationCode.SEPARATOR_
        ),
        ""
      )
      .toUpperCase();
    ...
    let normalLat =
      -OpenLocationCode.LATITUDE_MAX_ * OpenLocationCode.PAIR_PRECISION_;
    let normalLng =
      -OpenLocationCode.LONGITUDE_MAX_ * OpenLocationCode.PAIR_PRECISION_;
    let gridLat = 0;
    let gridLng = 0;
    // How many digits do we have to process?
    let digits = Math.min(
      editedCode.length,
      OpenLocationCode.PAIR_CODE_LENGTH_
    );
    ...
    for (let position = 0; position < digits; position += 2) {
      normalLat +=
        OpenLocationCode.CODE_ALPHABET_.indexOf(editedCode.charAt(position)) *
        pv;
      normalLng +=
        OpenLocationCode.CODE_ALPHABET_.indexOf(
          editedCode.charAt(position + 1)
        ) * pv;
    ...
    if (editedCode.length > OpenLocationCode.PAIR_CODE_LENGTH_) {
      ...
      for (let i = OpenLocationCode.PAIR_CODE_LENGTH_; i < digits; i++) {
        const digitVal = OpenLocationCode.CODE_ALPHABET_.indexOf(
          editedCode.charAt(i)
        );
    ...
    const lat =
      normalLat / OpenLocationCode.PAIR_PRECISION_ +
      gridLat / OpenLocationCode.FINAL_LAT_PRECISION_;
    const lng =
      normalLng / OpenLocationCode.PAIR_PRECISION_ +
      gridLng / OpenLocationCode.FINAL_LNG_PRECISION_;
  ...
  public static recoverNearest(
    ...
    const paddingLength =
      OpenLocationCode.SEPARATOR_POSITION_ -
      shortCodeUpper.indexOf(OpenLocationCode.SEPARATOR_);
    ...
    const codeArea = OpenLocationCode.decode(
      OpenLocationCode.encode(referenceLatitude, referenceLongitude).substr(
        0,
        paddingLength
      ) + shortCodeUpper
    );
    ...
    if (codeArea.codeLength! < OpenLocationCode.MIN_TRIMMABLE_CODE_LEN_) {
      throw new Error(
        "ValueError: Code length must be at least " +
          OpenLocationCode.MIN_TRIMMABLE_CODE_LEN_
      );
    }
  ...
  private static computeLatitudePrecision(codeLength: number): number {
    ...
    return (
      Math.pow(20, -3) / Math.pow(OpenLocationCode.GRID_ROWS_, codeLength - 10)
    );
  }
